...
from train import read_data, process_utter_label_list, utter_to_idx_list
...
def test():
    ...
    for test_file in test_file_list:
        ...
        for i, utter in enumerate(utter_list):
            utter_transformer = []
            for w in utter:
                if w not in vocab_dict.idx2word or vocab_dict.idx2word[w] == '<UNK>':
                    utter_transformer.append(0)
                utter_transformer.extend(text_encoder.encode([vocab_dict.idx2word[w]]))

            ctx_list.append(len(utter_transformer))
            utter_transformer += [0] * (n_ctx - len(utter_transformer))
            word_index = np.reshape(utter_transformer, [n_ctx, 1])
            word_pos = np.reshape(range(n_vocab, n_vocab + n_ctx), [n_ctx, 1])
            input_x_transformer.append(np.concatenate([word_index, word_pos], -1))
        # input_x_lstm = np.asarray(input_x_lstm, dtype=np.int)
        input_x_transformer = np.asarray(input_x_transformer, dtype=np.int)
        input_y = np.asarray(label_list, dtype=np.int)
        mask_list = np.asarray(mask_list, dtype=np.float)

        logits = sess.run([textRNN.logits], feed_dict={X_train: input_x_transformer,
                                                       textRNN.input_x: input_x_transformer[:, :, 0],
                                                       textRNN.input_y: input_y,
                                                       textRNN.mask_label: mask_list,
                                                       textRNN.seq_len_list: ctx_list})
...
if __name__ == '__main__':
        ...
        for utter in utter_list:
            ...
            for w in utter:
                if w not in vocab_dict.idx2word or vocab_dict.idx2word[w] == '<UNK>':
                    utter_transformer.append(0)
                utter_transformer.extend(text_encoder.encode([vocab_dict.idx2word[w]]))
            n_ctx = max(n_ctx, len(utter_transformer))
            # if len(utter) != len(utter_transformer):
            #     print([vocab_dict.idx2word[w] for w in utter], [text_encoder.decoder[w] for w in utter_transformer])
            #     count += 1
    ...
    embed_matrix = np.load('model/glove_dict.npy')
    glove_init = tf.constant_initializer(embed_matrix)
    ...
    textRNN = TextRNN(args.num_classes, args.learning_rate, args.decay_steps, args.decay_rate,
                      n_ctx, n_vocab, args.embed_size, args.hidden_size, args.dropout_keep_prob, is_training, tran_layer=n_layer+1, attention=False, input_embedding=input_embedding, glove_init=glove_init)
    ...
    for epoch in range(args.n_iter):
        ...
        for itData in data:
            ...
            # input_x_lstm = []
            for i, utter in enumerate(utter_list):
                # input_x_lstm.append(utter + [vocab_dict.word2idx['<PAD>']] * (max_seq_len - len(utter)))
                utter_transformer = []
                for w in utter:
                    if w not in vocab_dict.idx2word or vocab_dict.idx2word[w] == '<UNK>':
                        utter_transformer.append(0)
                    utter_transformer.extend(text_encoder.encode([vocab_dict.idx2word[w]]))
                # utter_transformer = text_encoder.encode([vocab_dict.idx2word[w] for w in utter])
                # input_x_lstm_ = []
                # for w in utter_transformer:
                #     input_x_lstm_.append(vocab_dict.word2idx[text_encoder.decoder[w]])
                # input_x_lstm_ += [vocab_dict.word2idx['<PAD>']] * (n_ctx - len(input_x_lstm_))
                # input_x_lstm.append(input_x_lstm_)
                ctx_list.append(len(utter_transformer))
                utter_transformer += [0] * (n_ctx - len(utter_transformer))
                word_index = np.reshape(utter_transformer, [n_ctx, 1])
                word_pos = np.reshape(range(n_vocab, n_vocab + n_ctx), [n_ctx, 1])
            ...
            loss, pred, _ = sess.run([textRNN.loss_val, textRNN.predictions, textRNN.train_op], feed_dict={X_train: input_x_transformer,
                                                                                                textRNN.input_x: input_x_transformer[:, :, 0],
                                                                                                textRNN.input_y: input_y,
                                                                                                textRNN.mask_label: mask_list,
                                                                                                textRNN.seq_len_list: ctx_list,
        ...
        saver.save(sess, 'transformer-lstm-model/model.ckpt', global_step=epoch)

        test_dir = 'eval/test_jsons'
        out_dir = 'eval/transformer-lstm-noattention/epoch{}'.format(epoch)
